...
export function checkPreWarn(candles: Candle[]): SignalResult | null {
  if (candles.length < 20) {
    return {
      ...
      explanation: 'Need 20+ candles for Pre-Warn analysis'
    };
  }
  ...
  if ((isOverboughtZone && isKCrossDownD) || isPriceBelowSMA10) {
    return {
      signal: SignalType.PRE_WARN,
      setupFamily: 'EXIT',
      explanation: isPriceBelowSMA10 ? "Price dropped below SMA10." : "Stochastic cross-down from overbought zone.",
      price: current.close,
  ...
  return {
    signal: SignalType.NONE,
    setupFamily: 'EXIT',
    rejectionReason: 'momentum_strong',
    explanation: 'Trend still holding above short-term MA'
  };
}
